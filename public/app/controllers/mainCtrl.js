...
// Controller: mainCtrl is used to handle login and main index functions (stuff that should run on every page)  
    .controller('mainCtrl', function (Auth, $timeout, $location, $rootScope, $window, $interval, User, AuthToken, $scope)
    {
        ...
        if ($window.location.pathname === '/')
            app.home = true; // Check if user is on home page to show home page div
        ...
        app.checkSession = function ()
        {
            ...
            if (Auth.isLoggedIn())
            {
                ...
                // Run interval every 30000 milliseconds (30 seconds)
                let interval = $interval(function ()
                {
                    let token = $window.localStorage.getItem('token'); // Retrieve the user's token from the client local storage
                    ...
                    else
                    {
                        ...
                        let expireTime = self.parseJwt(token); // Save parsed token into variable
                        let timeStamp = Math.floor(Date.now() / 1000); // Get current datetime timestamp
                        let timeCheck = expireTime.exp - timeStamp; // Subtract to get remaining time of token
        ...
        //Function that gets all user files
        app.getFiles = function ()
        {
            User.getAllFiles().then(function (files)
            {
                app.files = files.data.files;
            });
        };
    });
